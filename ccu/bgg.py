...
processor.chat_template = (
    ...
    "{% if messages[0]['role'] == 'system' %}"
        "{% set system_prompt_content = messages[0]['content'] %}"
        "{% set messages = messages[1:] %}"
    "{% else %}"
        "{% set system_prompt_content = system_prompt %}"
    "{% endif %}"
    "{{ '<|begin_of_text|>' }}"
    "{% if system_prompt_content != '' %}"
        "{{ '<|start_header_id|>system<|end_header_id|>\n\n' }}"
        "{% if system_prompt_content is string %}"
            "{{ system_prompt_content }}"
        "{% else %}"
            "{% for part in system_prompt_content %}"
                "{{ part }}"
            "{% endfor %}"
        "{% endif %}"
        "{{ '<|eot_id|>' }}"
